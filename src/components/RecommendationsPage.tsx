...
export default function RecommendationsPage() {
    ...
    async function fetchRecs() {
      try {
        const res = await fetch(
          "https://script.google.com/macros/s/AKfycbzv2MsUXQl1y6oShINZPdtkzZ6J9BQDbGlXhY-1ZYyV6FNqMl5NXRMR0ACsDntNaySi/exec"
        );
  ...
  if (loading) {
    return <p className="text-center text-black py-8">טוען המלצות...</p>;
  }
  ...
  return (
    <section className="max-w-5xl mx-auto px-4 py-10">
      ...
      <div
        className={`columns-1 sm:columns-2 md:columns-3 gap-4 "text-left"`}
      >
        {recs.map((rec, idx) => {
          const text =  rec.text;
          const name = rec.name;
